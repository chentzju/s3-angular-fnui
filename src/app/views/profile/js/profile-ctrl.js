angular.module("myApp").controller('InfoCtrl',['$scope','UserInfoService',
    ...
    .controller('addAddressCtrl',['$scope',
        function($scope){
            ...
            /*定义三级省市区数据*/
            $scope.province = 	["北京市", "天津市", "河北省", "河南省", "山西省", "山东省", "内蒙古自治区", "辽宁省", "吉林省", "黑龙江省", "上海市", "江苏省", "浙江省", "福建省", "江西省", "安徽省", "湖北省", "湖南省", "广东省", "广西壮族自治区", "海南省", "重庆市", "四川省", "贵州省", "云南省", "西藏自治区", "陕西省", "甘肃省", "青海省", "宁夏回族自治区", "新疆维吾尔自治区", "台湾省", "香港特别行政区", "澳门特别行政区"];
            city = 		[
                ["市辖区"],
                ["市辖区", "市辖县"],
            ...
            $scope.closeXaunQu=clockArea;
            $scope.areaBack=false;
            ...
            $scope.selectP=function (index) {
                $("#areaList1").scrollTop(0);
                $scope.areaBack=true;
                $scope.provinceShow=false;
                $scope.cityShow=true;
                $scope.districtShow=false;

                $scope.city=city[index];
                $scope.provinceIndex=index;
                expressArea+=$scope.province[index]+" > "
                console.log($scope.province[index])
                console.log($scope.provinceIndex)
            }
            $scope.selectC=function (index) {
                $("#areaList2").scrollTop(0);
                $scope.areaBack=true;

                console.log()
                $scope.district=district[$scope.provinceIndex][index]
                $scope.cityIndex=index

                var sCity = city[$scope.provinceIndex][index];
                if (sCity != "省直辖县级行政单位") {
                ...
                console.log(city[$scope.provinceIndex][index])
            }
            $scope.selectD=function(index) {
                $("#areaList").css("scrollTop",0)
                $scope.areaBack=true;
                /*clockArea();*/
                expressArea += district[$scope.provinceIndex][$scope.cityIndex][index];
                $("#expressArea dl dd").html(expressArea);
                clockArea()
                areaShow=true
            ...
            $(function() {
                ...
                $("#expressArea").click(function() {
                    ...
                    if(areaShow){
                        ...
                        console.log($scope.provinceShow,$scope.cityShow,$scope.districtShow)

                        console.log($scope.provinceShow)

                        areaShow=false;
                    }else{
                        $("#areaLayer").animate({"bottom": "-300px"});
                        areaShow=true;
                    }
                    /*$("#areaMask").fadeIn();*/
